...
const CreateTaskModal = ({ onClose, onSubmit, isLoading }) => {
  ...
  const validate = () => {
    const newErrors = {};
    if (!form.title.trim()) newErrors.title = t("task_title_required");
    return newErrors;
  };
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-[#2d2f33]">
          <h2 className="text-lg font-bold text-[#0A0A0A] dark:text-white">{t("create_task_title")}</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wide">
                {t("task_assigned_to_label")}
              </label>
              <input
                name="assignedTo"
                value={form.assignedTo}
                onChange={handleChange}
                placeholder={t("task_assigned_placeholder") }
          ...
          <div className="flex gap-3 pt-1">
            ...
            >
              {isLoading ? t("creating_ellipsis") : t("create_task_button")}
            </button>
          </div>
        </form>
      </motion.div>
